...
configuration.  It also features multiple access methods to its object storage, block storage, and file systems.  
...
RADOS GATEWAY
======================
The RADOS Gateway contains the ``radosgw``, which is an S3-compatible ReStructured Text (REST) HTTP service for object
storage.  This gateway uses the RADOS storage devices.
